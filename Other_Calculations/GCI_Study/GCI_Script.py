...
def main():
    with open(STAR_RESULTS_YAML) as fh:
        Analysis_Dict = yaml.load(fh, Loader=yaml.FullLoader)['Analysis']
    simulation_list = []
    Sim_List = []
    for item in Analysis_Dict:
        if item == 'Fluid_Volume':
            Fluid_Volume = Analysis_Dict['Fluid_Volume']
        else:
            Sim_Instance = Simulation(Analysis_Dict[item])
