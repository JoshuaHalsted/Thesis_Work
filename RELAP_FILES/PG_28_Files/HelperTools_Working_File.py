...
# What to do ?
generate_list_of_replecements = False
check_if_channels_are_in_file = False
write_input_file = False
write_strip_file = False
make_plots = True
# all plots of just the "production plots"?"
allPlots = False
Prod_Plots = False
# This only affects the "production plots"
# If True, titles are printed on the plots
printTitles = False
# If making plots, this also prints SS, max and slope values (for comparison tables in Report)
print_value_tables = False

test_names = ["Point_625","Point_5625","Point_5","Point_4375","Point_375","Point_3125","Point_25"]

### What kind of plots?

UPPER_COLD_DUCT = False
LOWER_COLD_DUCT = False
LOWER_RISER_TEMP = False
UPPER_PLENUM_AVG = False
BLOCK_7_INNER_RADIAL = False
BLOCK_7_INNER_REFLECTOR = False
BLOCK_7_OUTER_AVG_TEMP = False
BLOCK_7_MIDDLE_AVG_TEMP = False
BLOCK_7_INNER_AVG_TEMP = False
BLOCK_7_OUTER_REFLECTOR_AVG_TEMP = False
BLOCK_5_OUTER_AVG_TEMP = False
BLOCK_5_MIDDLE_AVG_TEMP = False
BLOCK_5_INNER_AVG_TEMP = False
BLOCK_5_INNER_REFLECTOR = False
BLOCK_5_INNER_FUEL_AVG = False
BLOCK_5_OUTER_REFLECTOR_AVG_TEMP = False
BLOCK_3_OUTER_AVG_TEMP = False
BLOCK_3_MIDDLE_AVG_TEMP = False
BLOCK_3_INNER_AVG_TEMP = False
BLOCK_3_INNER_REFLECTOR_AVG_TEMP = False
BLOCK_3_AVG_TEMPS = False
BLOCK_3_OUTER_REFLECTOR_AVG_TEMP = False
UPPER_HOT_DUCT_TEMP = False
LOWER_HOT_DUCT_TEMP = False
CORE_DT = False
DUCT_TEMPS = False
RCST_TEMPS = False
PRIMARY_SYSTEM_PRESSURE = False
CIRCULATOR_DP = False
CORE_DP = False
SG_INLET_FLOW = False
SG_INLET_TEMP = False
SG_DOME_PRESSURE = False
SG_LEVEL = False
RCCS_FLOW_RATE = False
RCCS_TEMPERATURES = False
RCCS_TEMP_DIFF = False

# Files
RELAP5_inp_file   = "RELAP_FILES/PG_28_Files/PG28_not_filled.i"
#RELAP5_inp_file   = "HTTF_base_PG26_splitduct_noSG.i"
RELAP5_out_file   = "RELAP_FILES/PG_28_Files/PG28_filled.i"
varlist_file      = "RELAP_FILES/PG_28_Files/R5_initial_cond_list"
instrument_map    = "RELAP_FILES/PG_28_Files/instrumentationmap2018-09-28.csv"
#Measured_data     = "Data/PG26_A_data.csv"
Measured_data     = "RELAP_FILES/PG_28_Files/PG28_Data_Quality.csv"
Measured_data_trend = "RELAP_FILES/PG_28_Files/PG28_Data_Trend.csv"
RELAP5_strip_file = "RELAP_FILES/PG_28_Files/Strip.i"
RELAP5_output     = "RELAP_FILES/PG_28_Files/Output_1"
#RELAP5_output     = ["RELAP_FILES/PG_28_Files/Output_Flow_Rate_1_Point_5.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_1_Point_25.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_1.o", "RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_75.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_5.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_375.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_625.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_4375.o","RELAP_FILES/PG_28_Files/Output_Flow_Rate_Point_5625.o"]



RELAP5_output_dir = "RELAP_FILES/PG_28_Files"
#RELAP5_output_dir = sys.argv[1]
#print("RUNNIG : " + RELAP5_output_dir)

# Some additional inputs....
# Takes the measured value  from this time stamp in the data file
#At which transient time should the ss be?
# => After the last valve movement befeor the bolwer is shut down (MS-6201)
# and the transient is strated (SV-6001)
# => at 6/1/19 at 8:10:40pm => 179500.0 s
runtime_for_initial_cond  = 0.0
end_runtime = 8000.0
# generate_list_of_replecements
# ----------------------------------------------------------------
# This reads the RELAP input and generates a list of
# all variables that need to be replaced (variables starting with "$")
if generate_list_of_replecements:
  varlist = []
  with open(RELAP5_inp_file) as R5in:
    for l in R5in:
      if "$" in l:
        for elem in l.split():
          if "$" in elem:
            varlist.append(elem)
  with open(varlist_file,'w') as VLin:
...
# It also indicates which RELAP channels in the input are in the mapping and which not
if check_if_channels_are_in_file:
  R5vars = pd.read_csv(varlist_file)
  instruments = pd.read_csv(instrument_map)
  # drop empty lines (rows)
  instruments.dropna(subset=['Tag_Number'], inplace=True)
  Measured = pd.read_csv(Measured_data)

  # Instruments listed in instrument_map in Measured data?
...
if write_input_file:
  ...
  Mapping = pd.read_csv(varlist_file)
  ...
  for to_be_replaced in varlist: 
    ...
    if to_be_replaced not in Mapping.RELAP_channel.values:
      print("For the variable on the line above, there is no rule in " + varlist_file )
      quit()
    ...
    for look_for_this in re.findall(r'\w\w\d\d\d\d' ,rep_rule):
      ...
      data_point =  Measured.loc[Measured['Run_Time'] == runtime_for_initial_cond, look_for_this_in_data].iloc[0]
  ...
  for heater in range(10):
    if (heater+1) == Heaters_used:
      #print("Updating heater " + str(heater+1))
      if heater+1 == 10:
        HeaterInstrumentNumber = '0'
      else:
        HeaterInstrumentNumber = str(heater+1)
      POW =  Measured['CT-10' + HeaterInstrumentNumber + '1'] * Measured['VT-10' + HeaterInstrumentNumber + '1'] + Measured['CT-10' + HeaterInstrumentNumber + '2'] * Measured['VT-10' + HeaterInstrumentNumber + '2']
      ...
      # No SMA for the portion when the DCC strats
      anf = 0
      end = 43888
  ...
  sort_val_indexes_Prim = sort_val_indexes_Prim.append(pd.Index(np.linspace(0, len(Measured['Run_Time'])-1, 50).astype(int)))
  #sort_val_indexes_Prim = sort_val_indexes_Prim.append(pd.Index([179950*2, 180100*2, 180300*2]))
  sort_val_indexes_RCST = sort_val_indexes_RCST.append(pd.Index(np.linspace(0, len(Measured['Run_Time'])-1, 50).astype(int)))
  ...
  Measured_trend = pd.read_csv(Measured_data_trend)
  # MASS FLOW RATE
  ...
  sort_val_indexes_Core_DT = sort_val_indexes_Core_DT.append(pd.Index(np.linspace(0, len(Measured['Run_Time'])-1, 50).astype(int)))
  ...
  sort_val_indexes_Core_T = sort_val_indexes_Core_T.append(pd.Index(np.linspace(0, len(Measured['Run_Time'])-1, 50).astype(int)))
  # 
  Ntime = [0.0]
  Nval  = [0.0]
  ...
  with open(RELAP5_out_file, 'w') as R5out:
    R5out.write(R5outTxt)

# 
# write_strip_file
# ----------------------------------------------------------------
# This writes a RELAP stip file to extract the needed data from the 
...
if write_strip_file:
  ...
  # Write Strip file
  with open(RELAP5_strip_file, 'w') as R5strip:
...
#  mylables = {'20kW_6000CFM': '20kW 2000CFM', '40kW_6000CFM': '40kW 6000CFM', '60kW_6000CFM': '60kW 6000CFM'}
  ...
  fout = open("RELAP_FILES/PG_28_Files/new.csv", "w")
  mytemp = ""
  mytempplotnum = ""
  with open(RELAP5_output) as fp:
    count = 0
    start = False
  ...
  R5data = pd.read_csv('RELAP_FILES/PG_28_Files/tempout.csv')
  #os.remove('RELAP_FILES/PG_28_Files/tempout.csv')
  ...
  if allPlots:
    for instrument in instruments['Tag_Number']:
      ...
      if savefigures == True:
        fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + instrument + '.png')
  ...
  if Prod_Plots:
    if UPPER_COLD_DUCT:
        ...
        if printTitles:
            title = plt.title("Upper Cold Duct Helium temperature")
        else:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_Cold_Duct_Upper.png')
    ...
    if LOWER_COLD_DUCT:
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Lower Cold Duct Helium temperature")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([250.0, 750.0])
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_Cold_Duct_Lower.png')
    ...
    if LOWER_RISER_TEMP:
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Upper Riser Helium Temperature")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([250.0, 750.0])
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_lower_riser.png')
    ...
    if UPPER_PLENUM_AVG:
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Upper Plenum Average Helium Temperature")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([250.0, 750.0])
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_upper_plenum_avg.png')

    # Primary fluid/solid core TOP (Block 7) 
    # = = = = = = = = = = = = = = = = = = = = = = = =
    if BLOCK_7_INNER_RADIAL:
        print("Primary fluid core Block 7 Middle")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Core Block 7 Inner Block Temps")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ...
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_Block_7_Inner_Temps.png')

    if BLOCK_7_INNER_REFLECTOR:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_TOPInnerRefl.png')

    if BLOCK_7_OUTER_AVG_TEMP:
        ...
        laby = plt.ylabel("Temperature [k]")
        if printTitles:
            title = plt.title("Core Block 7 Outer Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_7_Avg_Outer.png')

    if BLOCK_7_MIDDLE_AVG_TEMP:
        print("Primary solid core TOP fuel")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [k]")
        if printTitles:
            title = plt.title("Core Block 7 Middle Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_7_Avg_Middle.png')

    if BLOCK_7_INNER_AVG_TEMP:
        print("Primary solid core TOP fuel")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [k]")
        if printTitles:
            title = plt.title("Core Block 7 Inner Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_7_Avg_Inner.png')


    if BLOCK_7_OUTER_REFLECTOR_AVG_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_TOPOuterRefl.png')

    # Primary fluid/solid core Middle (Block 5) 
    # = = = = = = = = = = = = = = = = = = = = = = = =
    if BLOCK_5_OUTER_AVG_TEMP:
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Core Block 5 Outer Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsizeSmall)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_5_Avg_Outer.png')

    if BLOCK_5_MIDDLE_AVG_TEMP:
        print("Primary fluid core MID")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Core Block 5 Middle Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsizeSmall)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_5_Avg_Middle.png')

    if BLOCK_5_INNER_AVG_TEMP:
        print("Primary fluid core MID")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Core Block 5 Inner Average Solid Temperatures (Fuel)")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([200.0, 1800.0])
        ...
        leg = ax.legend(loc='best', fontsize=lfontsizeSmall)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_tempSOLID_core_Block_5_Avg_Inner.png')

    if BLOCK_5_INNER_REFLECTOR:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_MIDInnerRefl.png')

    if BLOCK_5_INNER_FUEL_AVG:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_MIDFuel.png')

    if BLOCK_5_OUTER_REFLECTOR_AVG_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_MIDOuterRefl.png')

    if BLOCK_3_OUTER_AVG_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsizeSmall)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_core_LOW.png')

    if BLOCK_3_MIDDLE_AVG_TEMP:
        # Primary fluid/solid core Low (Block 3) 
        # = = = = = = = = = = = = = = = = = = = = = = = =
        print("Primary fluid core LOW")
        fig = plt.figure(figsize=(figA,figB))
    ...
    if BLOCK_3_INNER_REFLECTOR_AVG_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_LOWInnerRefl.png')

    if BLOCK_3_AVG_TEMPS:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_LOWFuel.png')

    if BLOCK_3_OUTER_REFLECTOR_AVG_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempSOLID_core_LOWOuterRefl.png')

    # Primary fluid outlet temps
    # = = = = = = = = = = = = = = = = = = = = = = = =
    if UPPER_HOT_DUCT_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_core_Xoutlet.png')

    if LOWER_HOT_DUCT_TEMP:
        print("Primary fluid outlet lower duct temp")
        fig = plt.figure(figsize=(figA,figB))
        ax = fig.add_subplot(111)
        # plot the Measured data
        ...
        laby = plt.ylabel("Temperature [K]")
        if printTitles:
            title = plt.title("Lower Hot Duct Helium temperature")
        else:
            fig.subplots_adjust(bottom=BorderBot, right=BorderRig, top=BorderTop)
        xlim = plt.xlim(xlimites1)
        ylim = plt.ylim([300.0, 850.0])
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_core_Xloweroutlet.png')
    ...
    if CORE_DT:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_core_DT.png')
    ...
    if DUCT_TEMPS:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_SG_inout.png')
    ...
    if RCST_TEMPS:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_TempHE_RCST.png')

    # Primary and  RCST pressures
    # = = = = = = = = = = = = = = = = = = = = = = = =
    if PRIMARY_SYSTEM_PRESSURE:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_PressHE.png')
    ...
    if CIRCULATOR_DP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_DP_circ.png')
    ...
    if CORE_DP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_DP_core.png')
    ...
    if SG_INLET_FLOW:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_SG_massFlow.png')
    ...
    if SG_INLET_TEMP:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_SG_temp.png')
    ...
    if SG_DOME_PRESSURE:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_SG_press.png')
    ...
    if SG_LEVEL:
        ...
        leg = ax.legend(loc='best', fontsize=lfontsize)
        if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_SG_level.png')
    ...
    if RCCS_FLOW_RATE:
      ...
      try:
          ...
          if printTitles:
            ...
            leg = ax.legend(loc='best', fontsize=lfontsize)
          if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_RCCS_massFlow.png')
      except KeyError:
          pass
    ...
    if RCCS_TEMPERATURES:
      try:
          ...
          if printTitles:
            ...
            leg = ax.legend(loc='best', fontsize=lfontsize)
          if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_RCCS_temp.png')
      except KeyError:
          pass
    ...
    if RCCS_TEMP_DIFF:
      try:
          ...
          if printTitles:
            ...
            leg = ax.legend(loc='best', fontsize=lfontsize)
          if savefigures == True:
            fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_RCCS_tempDiff.png')
      except KeyError:
          pass
      ...
      if HEAT_BALANCE_PCS:
          ...
          #  scat = plt.plot(R5data['time-0'], R5data['cntrlvar-31'], 'bo-', markevery=200, label="cntrlvar-31 SG heat loss", markersize=10)
              ...
              if printTitles:
                ...
                leg = ax.legend(loc='best', fontsize=lfontsize)
              if savefigures == True:
                fig.savefig(RELAP5_output_dir + '/plots/Flow_Rate_5' + 'Prod_heatBalancePCS.png')
          except KeyError:
              pass
